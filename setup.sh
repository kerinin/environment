...
if [ -n $INSTALL ] && [ $OSTYPE=~darwin ]; then
  ...
  install_stuff ag 'apt-get install -y automake pkg-config libpcre3-dev zlib1g-dev; git clone git://github.com/ggreer/the_silver_searcher.git; cd .the_silver_searcher; ./build.sh; sudo make install; cd'
else
...
if [ $OSTYPE=~darwin ] && ! grep -Fxq "source ~/environment/.profile_osx" ~/.zshrc; then
  echo "source ~/environment/.profile_osx" >> ~/.zshrc
elif [ $OSTYPE=~linux ] && ! grep -Fxq "source ~/environment/.profile_ubuntu" ~/.zshrc; then
...
if [ $SHELL != /bin/zsh ]; then 
  echo "Changing shell from $SHELL to /bin/zsh - you'll need to enter your password"
